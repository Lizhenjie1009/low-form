...
v-if="(selectedWidget.type === 'radio') || ((selectedWidget.type === 'select') && !selectedWidget.options.multiple)"
    ...
    >
      <draggable
tag="ul" :list="optionModel.optionItems" item-key="id"
        v-bind="{
          group: 'optionsGroup',
          ghostClass: 'ghost',
          handle: '.drag-option'
        ...
        <template #item="{ element: option, index: idx }">
          <li>
            <el-radio :label="option.value">
              ...
              <el-button
circle plain size="small" type="danger" icon="el-icon-minus"
                         class="col-delete-button" @click="deleteOption(option, idx)"></el-button>
            </el-radio>
          </li>
        </template>
      </draggable>
...
v-model="optionModel.defaultValue" :options="optionModel.optionItems"
    ...
    >
      <el-dialog
v-model="showImportDialogFlag" :title="i18nt('designer.setting.importOptions')"
        :show-close="true"
    ...
    >
      <el-dialog
v-model="showImportCascaderDialogFlag" :title="i18nt('designer.setting.importOptions')"
        :show-close="true"
        class="drag-dialog small-padding-dialog"
        append-to-body
        :close-on-click-modal="false"
